...
public class EthClient {
    ...
    public BigInteger checkLatestMining() {
        try {
            ...
            List<PriceSheetView> list = nestMiningContract.list(erc20State.token.getAddress(), BigInteger.ZERO, Constant.BIG_INTEGER_FIVE, BigInteger.ZERO).send();
            if (list.size() == 0) {
            ...
            list = nestMiningContract.list(erc20State.token.getAddress(), Constant.BIG_INTEGER_FIVE, Constant.BIG_INTEGER_FIVE, BigInteger.ZERO).send();
            for (PriceSheetView priceSheetView : list) {
                if (priceSheetView.level.compareTo(BigInteger.ZERO) == 0) {
                    return priceSheetView.height;
                }
    ...
    /**
     * Bulk defrost token and Ntoken quotation sheet
     *
     * @param wallet
     * @param tokenAddress
     * @param tokenIndices
     * @param nTokenIndices
     */
    public String closeList2(Wallet wallet, BigInteger nonce, String tokenAddress, List<Uint256> tokenIndices, List<Uint256> nTokenIndices) {
        Credentials credentials = wallet.getCredentials();
        if (nonce == null) {
            log.error("{} ：closeList2 Failed to get nonce", credentials.getAddress());
            return null;
        }
        ...
        if (gasPrice == null) {
            log.error("closeList2 Failed to get GasPrice");
            return null;
        }

        List<Type> typeList = Arrays.<Type>asList(
                new Address(tokenAddress),
                new DynamicArray(Uint256.class, tokenIndices),
                new DynamicArray(Uint256.class, nTokenIndices));
        Function function = new Function("closeList2", typeList, Collections.<TypeReference<?>>emptyList());
        String encode = FunctionEncoder.encode(function);
        ...
        try {
            ...
            if (ethSendTransaction.hasError()) {
                Response.Error error = ethSendTransaction.getError();
                log.error("Closelist2 transaction failed to return：[msg = {}],[data = {}],[code = {}],[result = {}],[RawResponse = {}],",
                        error.getMessage(),
                        error.getData(),
                        error.getCode(),
                        ethSendTransaction.getResult(),
            ...
            log.error("Send Closelist2 transaction failed：{}", e.getMessage());
        }
        return transactionHash;
    }
